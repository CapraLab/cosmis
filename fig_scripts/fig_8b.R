...
kcn_monomer_cosmis <- read_tsv(
  file = paste(data_dir, "KCN_monomer_cosmis.tsv", sep = "/"), 
  col_names = TRUE,
  col_types = headers
) %>% mutate(
...
kcn_multimer_cosmis <- read_tsv(
  file = paste(data_dir, "KCN_multimer_cosmis.tsv", sep = "/"), 
  col_names = TRUE,
  col_types = headers
) %>% mutate(
  cosmis = (cs_mis_obs - mis_pmt_mean) / mis_pmt_sd,
...
interface_score_difference <-  ggplot() + 
  geom_histogram(
...
ggsave(
  filename = paste(figure_dir, "interface_score_difference.svg", sep = "/"),
  plot = interface_score_difference,
  width = 8,
  height = 6,
  units = "in",
  device = "svg",
