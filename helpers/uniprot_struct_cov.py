...
def parse_cmd_args():
    ...
    parser.add_argument(
        ...
        help='''A gzip compressed file UniProt protein sequences.'''
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='''A list of UniProt IDs and their corresponding 
...
def main():
    ...
    with gzip.open(cmd_args.input, 'rt') as handle:
        for record in SeqIO.parse(handle, cmd_args.format):
            ...
            # the second field is the UniProt ID, last field is length
            uniprot_id = id_fields[1] # retain the major part
            
            try:
                seq_length = len(record.seq)
            except:
                print('Invald length found in', uniprot_id, 'record!')
                continue
            
            protein_seqs[uniprot_id] = (seq_length, record.seq)
    ...
    coverages = []
    total_n_items = len(uniprot_to_pdb)
    completed = 0
    for k, v in uniprot_to_pdb.items():
        ...
        mapping = sifts_mapping.pdb_to_uniprot(pdb_id, chain_id, k)
        ...
        try:
            protein_length = protein_seqs[k][0]
        except KeyError:
            protein_length = None
        
        # compute and store the coverage
        try:
            coverage = pdb_mapped_length / protein_length
            ...
            continue
        coverages.append(
            [k, ''.join(v), protein_length, pdb_mapped_length, '%.3f' % coverage]
        )
    ...
    with open(cmd_args.output, 'wt') as opf:
        ...
        csv_writer.writerow(
            [
                'uniprot_id', 'pdb_chain', 'protein_length', 
                'covered_res', 'coverage'
            ]
        )
        for l in coverages:
