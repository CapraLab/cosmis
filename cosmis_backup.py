...
def get_codon_mutation_rates(cds):
    ...
    Returns
    -------
    mutation probabilities : tuple
        A tuple consisting of synonymous and nonsynonymous mutation probabilities.
    """
    if len(cds) % 3 != 0:
    ...
    for codon_number in range(1, num_codons + 1):
        ...
        # determine the mutation rate of the first codon
        if codon_number == 1 or codon_number == num_codons:
            if codon_number == 1:
                sequence_context = cds[:3]
            else:
...
def count_variants(variants):
    ...
    for variant in variants:
        ...
        if w != v:  # missense variant
            missense_counts[int(pos)] += int(ac)
        else:  # synonymous variant
            synonymous_counts[int(pos)] += int(ac)
        ...
        for transcript in ipf:
            ...
            with open(file=os.path.join(output_dir, transcript + '_mtr3ds.tsv'), mode='wt') as opf:
                header = ['TRANSCRIPT', 'PROTEIN', 'UNIPROT', 'PDB', 'CHAIN',
                    'ENSP_POS', 'ENSP_AA', 'PDB_POS', 'PDB_AA', 'CONTACTS',
